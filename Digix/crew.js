...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '22655752595@s.whatsapp.net'; // ton numéro ou le groupe cible
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '22655752595' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'ARMELBOT');
                console.log('📲 Pairing Code:', code);
                console.log('👉 Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['22655752595@s.whatsapp.net'], // emplace par ton numéro WhatsApp 
